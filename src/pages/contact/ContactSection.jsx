...
export default function ContactSection() {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    const res = await network.post(Url.createQuery, data);
    ...
    <>
      <div className="">
        <div className="grid grid-cols-12">
          <div className="bg-gray-900 col-span-12 md:col-span-4 p-10 text-white flex justify-center flex-col">
            <p className="mt-4 text-sm leading-7 font-regular uppercase">
              Contact
            </p>
            <h3 className="text-3xl sm:text-4xl leading-normal font-extrabold tracking-tight">
              Get In <span className="text-btn-color">Touch</span>
            </h3>
            ...
            <div className="flex items-center mt-5">
              <svg
                className="h-6 mr-2 text-btn-color"
                fill="currentColor"
                version="1.1"
          ...
          <form className="col-span-12 md:col-span-8 p-10">
            <div className="grid grid-cols-12 gap-4">
              {fields.map((field) => {
                if (field.type === "textarea") {
                  return (
                    <div
                      className={`${field.span || "col-span-12 md:col-span-6"}`}
                      key={field.name}
                    >
                      <TextAreaPlain
                        label={field.label}
